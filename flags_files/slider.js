    ...
    start_label, end_label)
{
    ...
    function changeSubmit(i, fade) {
        ...
        var code = newSubmit.find('pre.code');
        var cl = "";
        cl = 'brush: '+code.attr('data-sh-code');
        if (code.attr('data-sh-highlight'))
            cl += ';highlight: '+code.attr('data-sh-highlight');
        code.attr('class', cl);
   ...
   //     slider.slider('value', 0);
    ...
    function prevSubmit() {
        var i;
        for (var i = current-1; i >= 0 && sliderData[i].mode == ''; i--)
            ;
        if (i < 0)
            i = 0;
        jumpTo(i, true);
    ...
    var opened = false;
    var work = [];
    for(var i=0; i < task_changes.length; i++) {
    ...
    for (i = 0; i < sliderData.length; i++) {
        ...
        if (mode=='final' || mode=='verify' || mode=='paste')
            mclass=mode;
        var mark = $('<div class="submit-outer outer-'+mclass+'"></div>');
        var inner = $('<div class="submit-inner inner-' + mclass + '"></div>').appendTo(mark);
        timeline.append(mark);
        mark.attr('note','' + sliderData[i].timestamp);
        mark.css('left',
           time_to_percent(sliderData[i].timestamp) * timeline.width());
        inner.click(i, function(event) {
            jumpTo(event.data, true);
        });
    }
